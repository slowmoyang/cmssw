...
void GEMSimTrackMatch::bookHistograms(DQMStore::IBooker& ibooker, edm::Run const& run, edm::EventSetup const& iSetup) {
  ...
  for (unsigned int j = 0; j < nstation; j++) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      string sh_eta_title = sh_eta_name + "; track |#eta|; # of tracks";
      ...
      for (unsigned int k = 0; k < 3; k++) {
        ...
        string sh_phi_title = sh_phi_name + "; track #phi; # of tracks";
...
void GEMSimTrackMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& t : sim_trks) {
    ...
    FillWithTrigger(sh_eta, track_.gem_sh, std::abs(track_.eta));
    FillWithTrigger(sh_phi, track_.gem_sh, std::abs(track_.eta), track_.phi, track_.hitOdd, track_.hitEven);
  }
}
