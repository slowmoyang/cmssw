...
void GEMRecHitsValidation::bookHistograms(DQMStore::IBooker & booker,
  ...
  for (const auto & region : gem->regions()) {
    ...
    me_pull_x_[region_id] = bookHist1D(booker, region_id, "pull_x", "Pull of X", 100, -5, 5);
    me_pull_y_[region_id] = bookHist1D(booker, region_id, "pull_y", "Pull of Y", 100, -5, 5);
    ...
    if (detail_plot_) {
      for (const auto & station : region->stations()) {
        ...
        for (const auto & chamber : super_chamber->chambers()) {
          ...
          me_detail_pull_x_[key3] = bookHist1D(booker, key3, "pull_x", "Pull of X", 100, -5, 5);
          me_detail_pull_y_[key3] = bookHist1D(booker, key3, "pull_y", "Pull of Y", 100, -5, 5);
        } // chamber loop
      } // station loop
    } // detail plot
  } // region loop
  ...
  for (const auto & region : gem->regions()) {
    ...
    me_simhit_occ_eta_[region_id] = bookHist1D(booker, region_id,
                                               "muon_simhit_occ_eta",
                                               "Muon SimHit Eta Occupancy",
                                               50, eta_range_[0], eta_range_[1],
                                               "|#eta|");

    me_rechit_occ_eta_[region_id] = bookHist1D(booker, region_id,
    ...
    for (const auto & station : region->stations()) {
      ...
      me_simhit_occ_phi_[key2] = bookHist1D(booker, key2,
                                            "muon_simhit_occ_phi",
                                            "Muon SimHit Phi Occupancy",
                                            51, -M_PI, M_PI, "#phi");

      me_rechit_occ_phi_[key2] = bookHist1D(booker, key2,
                                            "matched_rechit_occ_phi",
                                            "Matched RecHit Phi Occupancy",
                                            51, -M_PI, M_PI, "#phi");

      me_simhit_occ_det_[key2] = bookDetectorOccupancy(booker, key2, station,
                                                       "muon_simhit",
                                                       "Muon SimHit");
      me_rechit_occ_det_[key2] = bookDetectorOccupancy(booker, key2, station,
                                                       "matched_rechit",
                                                       "Matched RecHit");
    ...
    if (detail_plot_) {
        ...
        for (const auto & chamber : super_chamber->chambers()) {
          ...
          me_detail_occ_xy_ch1_[key3] = bookXYOccupancy(booker, key3,
                                                        "rechit_ch1",
                                                        "(Chamber1) RecHits");
          // TODO me_detail_occ_xy_ch1_
          me_detail_occ_polar_[key3] = bookPolarOccupancy(booker, key3,
                                                          "rechit", "recHits");
