...
void GEMRecHitTrackMatch::bookHistograms(DQMStore::IBooker& ibooker,
  ...
  edm::LogInfo("GEMRecHitTrackMatch") << "GEM RecHitTrackMatch :: geom = " << gem_geom_ << std::endl;
  ...
  nstation = gem_geom_->regions()[0]->stations().size();
...
void GEMRecHitTrackMatch::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (const auto& t : sim_trks) {
    ...
    FillWithTrigger(rh_sh_eta, track_.gem_sh, std::abs(track_.eta));
    FillWithTrigger(rh_eta, track_.gem_rh, std::abs(track_.eta));

    // Separate station.

    FillWithTrigger(rh_sh_phi, track_.gem_sh, std::abs(track_.eta), track_.phi, track_.hitOdd, track_.hitEven);
    FillWithTrigger(rh_phi, track_.gem_rh, std::abs(track_.eta), track_.phi, track_.hitOdd, track_.hitEven);
  }
}
