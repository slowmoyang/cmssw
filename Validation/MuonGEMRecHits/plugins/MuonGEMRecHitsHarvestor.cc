...
void MuonGEMRecHitsHarvestor::dqmEndJob(DQMStore::IBooker& booker,
  ...
  for (const auto& region_id : region_ids_) {
    ...
    TString rechit_eta_path = gSystem->ConcatFileName(occ_folder, "matched_rechit_occ_eta" + name_suf_re);
    TString simhit_eta_path = gSystem->ConcatFileName(occ_folder, "muon_simhit_occ_eta" + name_suf_re);
    ...
    for (const auto& station_id : station_ids_) {
      ...
      // NOTE Phi
      TString rechit_phi_path = gSystem->ConcatFileName(occ_folder, "matched_rechit_occ_phi" + name_suf_re_st);
      TString simhit_phi_path = gSystem->ConcatFileName(occ_folder, "muon_simhit_occ_phi" + name_suf_re_st);
      ...
      TString rechit_det_path = gSystem->ConcatFileName(occ_folder, "matched_rechit_occ_det" + name_suf_re_st);
      TString simhit_det_path = gSystem->ConcatFileName(occ_folder, "muon_simhit_occ_det" + name_suf_re_st);
