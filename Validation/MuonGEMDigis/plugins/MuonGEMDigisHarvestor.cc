...
void MuonGEMDigisHarvestor::dqmEndJob(DQMStore::IBooker& booker,
  ...
  for (Int_t region_id : region_ids_) {
    ...
    TString strip_eta_path = gSystem->ConcatFileName(occ_folder, strip_eta_name);
    TString simhit_eta_path = gSystem->ConcatFileName(occ_folder, simhit_eta_name);
    ...
    for (Int_t station_id : station_ids_) {
      ...
      TString strip_phi_path = gSystem->ConcatFileName(occ_folder, strip_phi_name);
      TString simhit_phi_path = gSystem->ConcatFileName(occ_folder, simhit_phi_name);
      ...
      TString strip_det_path = gSystem->ConcatFileName(occ_folder, strip_det_name);
      TString simhit_det_path = gSystem->ConcatFileName(occ_folder, simhit_det_name);
